...
class LinkDAO extends DAO
{
    ...
    /*
     *
     * fonction qui retourne de nombre de lien
     *
     */

    public function nbrlien() {
        $sql = "
        ...
        if ($result){
            ...
            throw new \Exception("No link matching id " . $result);
        }
    ...
    /*
     *
     * fonction qui retourne un tableau de tableau de lien, les lien sont ranger en tableaux de page
     *
     */

    public function liensparpages($nombreDePage,$lienParPage) {
        $nombreDePage = 2;
        $toutleslien = [];
        $premiereEntree = 0;
        for ($i = 0; $i >$nombreDePage;$i++){
            $premiereEntree = ($i*$lienParPage);
            $sql = "
                SELECT * FROM tl_liens ORDER BY lien_id DESC LIMIT '.$premiereEntree.', '.$lienParPage.'
            ";
            $result = $this->getDb()->fetchAll($sql);
